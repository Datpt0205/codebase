"""Typed, fail-fast registries for workers and workflow graphs.
...
def parse_worker_file(path: Path) -> tuple[WorkerDefinition, str]:
    """Read a worker definition without requiring its graph to exist yet.

    A context needs its own worker settings to build the services it registers
    graphs with, which happens before the registry can resolve those graphs.
    """
    ...
    return definition, hashlib.sha256(raw_bytes).hexdigest()
...
class WorkerRegistry:
    ...
    def load_file(self, path: Path) -> LoadedWorker:
        definition, checksum = parse_worker_file(path)
        ...
        loaded = LoadedWorker(definition=definition, checksum=checksum, source_path=str(path))
