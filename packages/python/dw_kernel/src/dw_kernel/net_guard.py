...
def _is_private_address(host: str) -> bool:
    try:
        ...
        # `getaddrinfo` IDNA-encodes the host before it looks anything up, and
        # that step raises `UnicodeError` - not `gaierror` - for a label over 63
        # characters or an empty one. It is not an OSError either, so it escaped
        # this guard entirely: a scraped link with a malformed host took down the
        # whole run instead of being one page nobody could read. A host that
        # cannot be encoded cannot be resolved, so it gets the same verdict.
        raise OutboundUrlError(
            "outbound host is not a resolvable name", details={"host": host}
        ) from exc
