...
def test_safe_attributes_redacts_and_coerces() -> None:
    ...
    assert cleaned["dw.worker_id"] == "demo"
...
def test_null_telemetry_is_silent() -> None:
    ...
    with telemetry.span("dw.run.start", {"a": 1}):
        telemetry.add_metric("dw_run_total", 1, {"worker": "demo"})
...
def test_otel_telemetry_exports_spans_and_metrics() -> None:
    ...
    with telemetry.span("dw.run.start", {"dw.worker_id": "demo", "token": "secret!"}):
        pass
    telemetry.add_metric("dw_run_total", 1, {"worker": "demo", "status": "completed"})
    telemetry.add_metric("dw_run_total", 1, {"worker": "demo", "status": "completed"})

    spans = span_exporter.get_finished_spans()
    assert [s.name for s in spans] == ["dw.run.start"]
    assert spans[0].attributes is not None
    assert spans[0].attributes["dw.worker_id"] == "demo"
...
def test_recording_telemetry_captures_for_assertions() -> None:
    ...
    with telemetry.span("dw.run.start", {"dw.worker_id": "demo"}):
        pass
    telemetry.add_metric("dw_run_total", 1, {"worker": "demo"})
    assert telemetry.spans == [("dw.run.start", {"dw.worker_id": "demo"})]
    assert telemetry.metrics == [("dw_run_total", 1, {"worker": "demo"})]
